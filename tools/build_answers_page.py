...
<li><b>How many clusters.</b> There is no correct answer here, what we discussed in class might help you reflect on what is valuable. But overall if you get groups of genes that co-express only under the conditions you are interested in you have found a solid start.</li>
...
<li><b>Why that makes biological sense:</b> the plant has a limited budget of energy, carbon and nitrogen, and both responses are expensive. Under a real, prolonged drought the immediate threat to survival is water loss, so it makes sense to spend everything on closing stomata, adjusting osmotic balance and protecting tissue, even though the price is a temporarily lowered guard against attackers. The two branches are not independent processes that happen to interfere; the repression of ICS1 by the ABA branch looks like a deliberate switch that <em>prioritises</em> one stress over the other. Combine this with the previous step and you get the full logic: brief dry spells are ignored, and only a drought that persists is worth paying for. The uncomfortable consequence for agriculture is that a drought-stressed crop is a more vulnerable crop, which is exactly the trade-off breeders would like to break.</li>
...
# --- Step 5: knockouts ---------------------------------------------------------
"""<ul>
<li><b>Best knockouts:</b> silence the two repressors of ICS1, that is <b>ANAC019 and GENEC together</b>. Then even a long drought cannot switch ICS1 off, so SA and BGL2 stay high and the plant keeps its white-fly defence throughout. Knocking out <b>ABA</b> also works <em>in the model</em>, by removing the drought signal before it ever reaches the repressors, but hold that thought until the next question. Note the trap: knocking out ICS1, SA or BGL2 themselves <em>destroys</em> defence, the opposite of what you want.</li>
<li><b>Realistic?</b> Not straightforwardly. Defence is metabolically costly, so a plant that can never dial it down may waste resources it needs to survive drought, hurting growth or yield, a cost this model does not include.</li>
<li><b>The ABA knockout in particular is a bad idea,</b> and it is worth being specific about why, because in the model it looks like the cleanest solution of all. In the diagram ABA is drawn as a single node with two outgoing arrows, as though its only job were to feed this pathway. In a real plant ABA is one of the central hormones, and it is <em>pleiotropic</em>, meaning one molecule with many different jobs. It is what closes the stomata when water runs short, which is the plant's single most important drought response, so a plant without ABA keeps its pores wide open and dries out faster than a normal plant. It also controls seed dormancy, so seeds germinate at the wrong moment, and it feeds into cold and salt tolerance, root growth and leaf senescence. So a knockout that the model scores as a success (defence stays high) would in reality give you a plant that keeps its white-fly resistance and then dies of the drought. The general lesson: the model contains only the arrows somebody drew, and a node's real biological connections are almost always broader than the ones in the diagram. The more central and upstream a component is, the more true that is, which is why intervening on a specific downstream regulator is usually safer than intervening on a hormone.</li>
<li><b>Other things the model leaves out.</b> Transcription factors such as ANAC019 are pleiotropic too, just less spectacularly so than ABA, so they carry the same risk on a smaller scale. Beyond that: off-target edits, redundancy between genes (another gene may quietly take over the repression), differences between tissues and developmental stages, and the fact that a greenhouse is not a field.</li>
...
<li><b>Clicking the tallest bars.</b> No, none of them does. Whichever bar you pick, the two clouds of points overlap heavily: plants with a 0 and plants with a 1 both spread over almost the whole range of yields, and the two group averages (the white lines) sit only a little apart. Compare that with Marker 12 in step 2, where the two groups formed two separate bands with a clear gap between them. The yield markers are not fake, each one really does shift the average slightly, but on its own each explains only a small part of why one plant yields more than another. That small shift is exactly why these bars stay modest instead of towering like Marker 12.</li>
...
"""<ul>
<li><b>Comparing the two plots.</b> The salt-resistance plot had one tall spike at Marker 12 that rose further and further above every other bar as the population grew. This one never produces that spike. Instead a group of markers at the left each reach a modest height. Growing the population makes the pattern cleaner but does not turn any bar into a winner, so this is not a problem you can solve by collecting more plants.</li>
...
<li><b>Epistasis.</b> No, this model cannot capture it. The prediction is one fixed weight per marker, added up, so the marker for the improved second enzyme contributes the same amount whether or not the plant also carries the improved first one, and there is no term that can switch on only when both are present. The model does the best it can by splitting the joint effect across the two weights, which is roughly right on average but wrong for individual plants: it overestimates plants carrying only one of the pair and underestimates plants carrying both. That matters for breeding, because the allele combinations worth chasing are exactly the ones the model cannot see.</li>
</ul>""",
]
...
"""<ul>
<li><b>The three treatments and why compare them.</b> <b>MOCK</b> is the control: the plant is infiltrated with liquid but receives no bacteria. <b>AVR</b> is infection with an avirulent strain, one the plant recognises and successfully fights off. <b>VIR</b> is infection with a virulent strain, one that evades the plant's defences and causes disease. Comparing either infection against MOCK tells you what the bacteria did rather than what the handling did, and comparing AVR against VIR tells you what a <em>successful</em> defence looks like.</li>
<li><b>Why several time points.</b> A defence response is a sequence of events, not a state: the plant has to detect the bacteria, pass the signal on, switch on transcription factors and only then make defence proteins, and those stages are hours apart. A single snapshot catches one frame of that with no way of knowing which. Several time points show the order in which genes come on, and protect you from sampling too early or too late.</li>
</ul>""",

# --- Step 2: reading the clustered heatmap ------------------------------------------
"""<ul>
<li><b>Rows, columns and colours.</b> Each <b>row</b> is one of the 500 genes, each <b>column</b> is one of the 9 samples (one treatment at one time point), and the <b>colour</b> is that gene's z-scored expression: red means above that gene's own average, blue below, pale about average. Because the scores are computed per gene, colours are only comparable <em>along</em> a row, so a red cell does not mean the gene is highly expressed in absolute terms.</li>
<li><b>Is AVR closer to VIR or to MOCK? To VIR.</b> The dendrogram shows it twice. At 1 hour, AVR_1 and VIR_1 merge with each other first and MOCK_1 joins only afterwards. Among the later samples, the AVR pair and the VIR pair join <em>each other</em> before the MOCK pair joins them. Once you stop reading the names this makes sense: avirulent and virulent sound like opposites, but from the plant's point of view both mean bacteria are present, and MOCK is the odd one out because nothing is attacking it.</li>
<li><b>Treatment or time point? Both, at different levels.</b> The very first split separates the three 1-hour samples from all six later ones, so at the coarsest level <b>time</b> dominates. Inside the later group the structure is entirely by <b>treatment</b>, with AVR pairing with AVR and so on. Note that all three 1-hour samples cluster together, MOCK included, so whatever makes them distinctive cannot be the bacteria: it is the handling, wounding and infiltration every plant went through, plus the time of day.</li>
<li><b>The clustering in your own words.</b> Something like: the algorithm knew nothing about treatments or time points, it only saw 9 columns of numbers, and it still recovered the design of the experiment. That doubles as a sanity check, because if matching conditions had <em>not</em> grouped together you would suspect a mislabelled sample or a batch effect long before interpreting any individual gene.</li>
</ul>""",

# --- Step 3: choosing a clustering --------------------------------------------------
"""<ul>
<li><b>How many clusters, and is there a right answer? No, and that is the point.</b> Four is defensible at the default settings, but so are other choices, and the decision is a judgement rather than a calculation. The real test is whether the groups you get are interpretable and whether your conclusions survive a change of settings: if a story only appears at one particular value of k, it is not a story. Decide what you would pick and why, and bring that to class so we can compare what different people chose.</li>
<li><b>What changes when you switch the settings.</b> <b>Linkage</b> matters enormously. <em>Average</em> and <em>complete</em> both give sensibly sized groups, while <em>single</em> collapses: at k = 4 it gives one cluster of 497 genes and three single-gene clusters. That is the classic failure called <b>chaining</b>, because single linkage merges two clusters as soon as their <em>closest</em> members are close, so one gene sitting between two groups is enough to fuse them. <b>Distance</b> barely matters here, because the values are already z-scored per gene, which makes Euclidean distance close to a rescaling of correlation distance. On raw expression the two would differ a great deal.</li>
<li><b>Which cluster to follow up, and how to decide.</b> A cluster of genes that rises in AVR at 6 and 12 hours is the best candidate. The criterion is not that it changes a lot, but that it changes <em>where the biology is interesting</em>: it is high in the treatment where the plant successfully defends itself, much weaker in VIR, and flat in MOCK, so the handling cannot explain it.</li>
<li><b>Linking a cluster to the biology.</b> Be honest about what a profile can do: it tells you <em>when</em> and <em>in which treatment</em> genes are active, so it lets you propose a function, not establish one. A cluster high at 1 hour in <em>all three</em> treatments cannot be about the bacteria, since MOCK had none, so those genes are more likely responding to handling and wounding or tracking the light. A cluster that rises in AVR but stays flat in MOCK is switched on only when the plant detects an attacker, so defence is the obvious guess. A cluster climbing late in every treatment looks more like general housekeeping. Each of those guesses came from <em>where</em> the block of colour sits rather than from the genes; the gene list under the figure lets you check a handful by hand, and doing it for hundreds at once is what the next practical is for.</li>
</ul>""",

# --- Step 4: the correlation network ---------------------------------------------------
"""<ul>
<li><b>What the threshold does.</b> It sets how strong a correlation has to be before two genes are joined. At 0.85 almost everything is connected, roughly 15,000 edges among 489 genes, and the result is a hairball; at 0.95 about 2,900 edges among 378 genes leave distinct knots visible; at 0.99 only about 156 edges among 118 genes survive. The threshold is a choice you make, not something the data tells you, so check that the group you care about holds together across a range of values.</li>
<li><b>What an edge means biologically.</b> That the two genes rise and fall together across the nine samples, which is evidence they are <b>co-regulated</b>, perhaps by the same signal or as part of the same pathway. It is not evidence that the two proteins touch, that one controls the other, or that either is causal. With only 9 samples and a response as strong as the 1-hour handling effect, plenty of pairs will correlate for reasons that have nothing to do with a shared function. Correlation networks generate hypotheses; they do not test them.</li>
<li><b>Would you expect a relationship? Yes.</b> Both views are built from the <em>same</em> correlations between the same genes, so a group that really does move together has to show up in both, as a branch of the tree and as a knot in the web. If they came out completely unrelated, the conclusion would not be "interesting biology" but that one of the two is being driven by its settings. Expect the correspondence to be good but not exact, since a cut of the tree and a threshold on the edges are two different ways of drawing a line.</li>
<li><b>What each view gives you that the other cannot.</b> Only from the <b>network</b>: a gene may belong to no group at all, or sit between two knots and belong to both, which is how you spot something linking two processes; it also gives you a <em>number</em> per gene describing where that one gene sits in relation to all the others, such as its <b>degree</b> (how many genes it is directly tied to, so whether it is a hub) or its <b>betweenness</b> (how often it lies on the path between other genes, so whether it is a bridge). Clustering cannot give you that at all: it tells you which group a gene is in, never where it sits within that group. Sliding the threshold also acts as a robustness check. Only from <b>clustering</b>: a complete partition, every gene in exactly one group, which is what you need to count anything or run an enrichment test on a defined list; a <em>hierarchy</em> rather than one slice; and an ordering of the <em>samples</em>, which is what let you read the design off the heatmap in step 2. Two views of one correlation matrix, so trust the structure that survives in both.</li>
<li><b>Which genes to study next, and what experiment.</b> Say why, rather than just naming the top of the list: a gene with high <em>degree</em> is correlated with many others so may sit at the heart of a module, while high <em>betweenness</em> puts it on the paths between knots, making it a candidate for connecting two processes. Both are hypotheses about position in a graph, not measured facts. Concretely: take two or three central genes from the knot matching the AVR-specific cluster, check what is known about them in UniProt, obtain knockout and overexpression lines, repeat the infection with AVR, VIR and MOCK, and measure the disease along with the expression of the genes they were correlated with. If knocking the gene out flattens its neighbours, you have evidence it is upstream rather than merely correlated.</li>
</ul>""",
]
